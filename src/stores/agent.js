...
export const useAgentStore = defineStore('agent', {
    ...
    getters: {
        maxQuoteThisRound (state) {
            if(state.currentAgent.awe_agent.awe_token_config.max_token_per_round >= state.currentAgentStats.awe_token_quote) {
                return state.currentAgentStats.awe_token_quote + state.currentAgentStats.awe_token_round_transferred
            } else {
        ...
        roundUsedPercentage (state) {
            if (this.maxQuoteThisRound === 0) {
                return 0
            } else if (state.currentAgent.awe_agent.awe_token_config.max_token_per_round >= state.currentAgentStats.awe_token_quote) {
                return Math.floor( state.currentAgentStats.awe_token_round_transferred * 100 / (this.maxQuoteThisRound + state.currentAgentStats.awe_token_round_transferred))
            } else {
                return Math.floor( state.currentAgentStats.awe_token_round_transferred * 100 / state.currentAgent.awe_agent.awe_token_config.max_token_per_round)
            }
        },
        tgBotReady (state) {
            return state.currentAgent.tg_bot.username !== ""
        ...
        tokenReady (state) {
            return this.maxQuoteThisRound !== 0
                && this.roundUsedPercentage < 100
        ...
        async loadAgentData (agentId) {
            ...
            if (loadedAgentData.awe_agent) {
                ...
                if (loadedAweAgent.image_generation_args) {
                    Object.assign(currentAgent.awe_agent.image_generation_args, loadedAweAgent.image_generation_args)
